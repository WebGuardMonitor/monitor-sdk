        ...
        WINDOW.addEventListener(
            ...
            (event: PromiseRejectionEvent) => {
                if (event.type === 'unhandledrejection') {
                    bindReporter(
                        constructReportData(PROMISE_TYPE, {
                            event,
                            message:
                                event.reason instanceof ReferenceError || event.reason instanceof TypeError
                                    ? event.reason?.message
                                    : event.reason,
                            stack:
                                event.reason instanceof ReferenceError ||
                                (event.reason instanceof TypeError && event.reason?.stack?.split('\n')) ||
                                [],
                        }),
                    );
                }
